...
> **Status: complete.** This project is finished and intentionally frozen at its final result: the best custom kernel reaches **5.84x over the naive kernel and 55.1% of cuBLAS SGEMM** throughput on the test GPU. The [closing analysis](#closing-analysis-why-this-project-stops-at-55-of-cublas) explains why this is a deliberate stopping point rather than an unfinished one.
...
- [4x4 TILE_K=32 Double-Buffered Shared-Memory Nsight Compute report](profiling/GEMMRegisterTile44TILEK32_DB_NsightCompute.pdf)
...
## Closing Analysis: Why This Project Stops at 55% of cuBLAS

The RTX 4060 Laptop GPU (AD107, 3072 CUDA cores) has a nominal FP32 peak of roughly `14.6 TFLOP/s` at full boost (`3072 cores x 2 FLOP x ~2.37 GHz`), which laptop power limits rarely sustain. Against that ceiling, cuBLAS reaches `5.86 TFLOP/s` (~40% of nominal peak, consistent with a power-capped mobile part) and the best custom kernel reaches `3.23 TFLOP/s`.

The remaining gap is **not memory bandwidth**. At `M = N = K = 4096` with `64 x 64` block tiles the kernel is firmly compute-bound: Nsight Compute reports only `13.13%` DRAM throughput for the best kernel. The gap lives inside the SM — instruction mix and latency hiding — and closing it would require, in rough order of impact:

1. **Vectorized 128-bit loads (`float4` / `LDS.128`).** Every load in this project is a scalar 32-bit load, so the kernels issue ~4x more load instructions than cuBLAS-style kernels, stealing issue slots from the FFMA pipe.
2. **True load/compute overlap via `cp.async`.** The double-buffering experiment already demonstrated this negatively: without `cp.async`, doubling the shared-memory buffers made the kernel *slower* (`46.20 ms` vs `42.54 ms`), because global loads still route through registers and cannot genuinely overlap with computation.
3. **Warp-level tiling and a bank-conflict-free (swizzled) shared-memory layout.** cuBLAS/CUTLASS kernels decompose the block tile into warp tiles for better register reuse and ILP; the `Btrans` experiment (3.8x slowdown from one shared-memory layout change) shows how sensitive this level is.
4. **Per-shape kernel selection.** `42.54 ms` is one hand-picked tile configuration; cuBLAS dispatches architecture- and shape-specific kernels (here it launched `ampere_sgemm_64x64_nn`).

Implementing all four faithfully converges on re-implementing CUTLASS — a well-documented path with diminishing educational return per step.

There is also a more fundamental reason to stop here: **production LLM-inference GEMM does not run on this code path at all.** It runs bf16/fp8 on Tensor Cores (`mma`/`wgmma`) through cuBLASLt, CUTLASS, or Triton, where Tensor-Core throughput is several times the FP32 SIMT rate — no amount of SIMT tuning is competitive with switching hardware paths. What transfers from this project is the method, not the kernel: profile, form a hypothesis, change one variable, and let Nsight Compute confirm or refute it. Several experiments here exist precisely because they *failed* informatively (`SmemPad`, `Btrans`, `Block32x8`, double buffering without `cp.async`).
